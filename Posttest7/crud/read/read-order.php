...
if ($_SESSION['name'] === 'admin') {
    $sql = "SELECT * FROM orders WHERE fullname LIKE '%$search%' OR phone_number LIKE '%$search%'";
} else {
    $sql = "SELECT * FROM orders WHERE user_id = $user_id AND fullname LIKE '%$search%' OR phone_number LIKE '%$search%'";
}
    ...
    <div class="header">
        <div class="btn"> 
            ...
            <?php if (isset($_SESSION['name']) && $_SESSION['name'] !== 'admin') { ?>
                <a href="../../order.php" style="margin-left: 10px;"><button>Back</button></a>
            <?php } ?>
        </div>
        <div class="search">
